...
function tem_o_ciclone() {
    ...
    // Calcular composição de fundo
    let sum_comp_fundo = sum_array(faixa_eficiencias);

    let comp_fundo = [];

    for (i = 0; i < dp.length; i++) {
        comp_fundo.push(Math.round(faixa_eficiencias[i] * 10000 / sum_comp_fundo) / 100 + "%");
    }
    ...
    for (i = 0; i < dp.length; i++) {
        faixa_ef_topo.push(1 - faixa_eficiencias[i]);
    }

    let sum_comp_topo = sum_array(faixa_ef_topo);

    let comp_topo = [];

    for (i = 0; i < dp.length; i++) {
        comp_topo.push(Math.round(faixa_ef_topo[i] * 10000 / sum_comp_topo) / 100 + "%");
    }
    ...
    document.getElementById("composicao_fundo").innerHTML = makeTableHTML("fundo", [analise_granulometrica()[0], comp_fundo]);
    document.getElementById("composicao_topo").innerHTML = makeTableHTML("topo", [analise_granulometrica()[0], comp_topo]);
